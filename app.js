...
- By TekTrey / TEKDEV
- 
- Hugging Face Space: https://huggingface.co/spaces/Shinhati2023/zontzend
- ============================================================
  */
...
const PROMO_CODES = { TREY10: 0.10, BURGER20: 0.20, NEON: 0.15 };
...
{
id: “p1”, name: “Flame Burger”, category: “Burgers”, price: 9.99,
...
{
id: “p2”, name: “Double Cheese Burger”, category: “Burgers”, price: 11.49,
...
{
id: “p3”, name: “Crispy Chicken Burger”, category: “Burgers”, price: 10.49,
...
{
id: “p4”, name: “Orange Neon Soda”, category: “Drinks”, price: 3.49,
...
{
id: “p5”, name: “Iced Coffee”, category: “Drinks”, price: 4.29,
...
{
id: “p6”, name: “Mango Drink”, category: “Drinks”, price: 3.99,
...
{
id: “p7”, name: “Loaded Fries”, category: “Fries”, price: 6.49,
...
{
id: “p8”, name: “Onion Rings”, category: “Snacks”, price: 5.49,
...
{
id: “p9”, name: “Chocolate Cake”, category: “Desserts”, price: 5.99,
...
{
id: “p10”, name: “Vanilla Cup”, category: “Desserts”, price: 4.49,
...
orderCount = parseInt(localStorage.getItem(“tb_orders”) || “0”);
}
...
// Three.js particle field for splash
...
const cam = new THREE.PerspectiveCamera(70, window.innerWidth / window.innerHeight, 0.1, 100);
cam.position.z = 5;
...
const mat = new THREE.PointsMaterial({ color: 0xff7a00, size: 0.06, transparent: true, opacity: 0.7 });
scene.add(new THREE.Points(geo, mat));
let t = 0;
function animSplash() {
if (!document.getElementById(“splash-screen”) || document.getElementById(“splash-screen”).style.display === “none”) return;
t += 0.004;
...
animSplash();
} catch (e) { /* WebGL not available, splash still works */ }

// Typewriter sequence
const lines = [
{ el: document.getElementById(“tl1”), text: “TREY FOOD SHELL”, speed: 55 },
{ el: document.getElementById(“tl2”), text: “BIG”, speed: 90, big: true },
{ el: document.getElementById(“tl3”), text: “BETTER”, speed: 90, big: true },
{ el: document.getElementById(“tl4”), text: “BEST”, speed: 90, big: true },
{ el: document.getElementById(“tl5”), text: “We live for Burgers.”, speed: 60 }
];
let delay = 300;
lines.forEach((line, idx) => {
setTimeout(() => {
line.el.classList.remove(“hidden”);
setTimeout(() => line.el.classList.add(“visible”), 20);
const textEl = line.el.querySelector(”.terminal-text”);
if (textEl) typewrite(textEl, line.text, line.speed, idx === 0);
...
else if (cursor && !showCursor) setTimeout(() => cursor.style.display = “none”, 300);
}
tick();
}
...
// Build low-poly cloth mesh (grid of lines)
...
for (let r = 0; r <= rows; r++) {
  ...
  for (let c = 0; c <= cols; c++) {
    ...
    pts.push(new THREE.Vector3(verts[idx*3], verts[idx*3+1], 0));
  }
  const geo = new THREE.BufferGeometry().setFromPoints(pts);
  const mat = new THREE.LineBasicMaterial({
    color: 0xff7a00,
    transparent: true,
    opacity: 0.25 - r * 0.015
  });
  group.add(new THREE.Line(geo, mat));
}

// Vertical lines (every 3rd for lightness)
...
function animHero() {
  ...
  for (let r = 0; r <= rows; r++) {
    ...
    if (!pos) continue;
    for (let c = 0; c <= cols; c++) {
      const x = origPos[(r*(cols+1)+c)*2];
      const wave = Math.sin(t + c * 0.35 + r * 0.5) * 0.18
...
} catch (e) { /* fallback — no canvas */ }
}

/* ============================================================
LIQUID RIPPLE (canvas 2D fallback — lightweight)
============================================================ */
...
ripples.push({ x, y, r: 0, maxR: 80, alpha: 0.5, t: 0 });
if (ripples.length === 1) animRipples(ctx);
}
...
if (ripples.length) requestAnimationFrame(() => animRipples(ctx));
...
if (textEl) textEl.textContent = “HF Backend…”;
try {
const res = await fetch(`${API_BASE}/health`, { signal: AbortSignal.timeout(6000) });
if (res.ok) {
el.classList.add(“online”);
if (textEl) textEl.textContent = “Backend Online”;
} else throw new Error(“non-ok”);
} catch {
el.classList.add(“sleeping”);
if (textEl) textEl.textContent = “Backend Sleeping”;
}
}
...
try {
const res = await fetch(`${API_BASE}/api/products`, { headers: getAuthHeaders(), signal: AbortSignal.timeout(5000) });
...
method: “POST”, headers: getAuthHeaders(), body: JSON.stringify(product),
...
method: “PUT”, headers: getAuthHeaders(), body: JSON.stringify(product),
signal: AbortSignal.timeout(5000)
});
if (!res.ok) throw new Error();
return await res.json();
...
try {
const res = await fetch(`${API_BASE}/api/products/${id}`, {
method: “DELETE”, headers: getAuthHeaders(), signal: AbortSignal.timeout(5000)
});
return res.ok;
} catch { return false; }
}
...
if (name === “profile”) updateOrderCount();
...
if (!filtered.length) {
grid.innerHTML = ` <div style="grid-column:1/-1;text-align:center;padding:40px 0;color:var(--text-muted);"> <div style="font-size:40px;margin-bottom:12px;">🔍</div> <p style="font-size:14px;">No items found</p> </div>`;
return;
}
...
return `<div class="product-card ${p.available ? "" : "unavailable"}" data-id="${p.id}"> <div class="product-img-wrap"> <img src="${p.image}" alt="${p.name}" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=200&q=60'" /> <button class="fav-btn ${isFav ? "active" : ""}" data-id="${p.id}" aria-label="Favorite ${p.name}"> <i data-lucide="heart"></i> </button> ${!p.available ?`<div style="position:absolute;inset:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;font-size:11px;font-family:var(--font-mono);color:#ff4d6d;letter-spacing:.05em;">SOLD OUT</div>`: ""} </div> <div class="product-info"> <div class="product-name">${p.name}</div> <div class="product-cat">${p.category}</div> <div class="product-footer"> <span class="product-price">$${parseFloat(p.price).toFixed(2)}</span> ${p.available ?`<button class="add-btn" data-id="${p.id}" aria-label="Add ${p.name} to cart"><i data-lucide="plus"></i></button>` : ""} </div> </div> </div>`;
}
...
const isFav = favorites.has(p.id);
content.innerHTML = `<div class="modal-product-img"> <img src="${p.image}" alt="${p.name}" onerror="this.src='https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=400&q=60'" /> </div> <div class="modal-product-name">${p.name}</div> <div class="modal-product-cat">${p.category}</div> <p class="modal-product-desc">${p.description || "A delicious item from Trey Burgers."}</p> <div class="modal-product-footer"> <span class="modal-price">$${parseFloat(p.price).toFixed(2)}</span> ${p.available ?`<button class="modal-add-btn" id="modal-add-btn" data-id="${p.id}">
<i data-lucide="plus"></i> Add to Cart
</button>`:`<span style="font-size:12px;color:#ff4d6d;font-family:var(--font-mono);">SOLD OUT</span>`} </div> <div style="display:flex;gap:10px;margin-top:14px;"> <button class="btn-ghost" id="modal-fav-btn" data-id="${p.id}" style="flex:1;display:flex;align-items:center;justify-content:center;gap:8px;"> <i data-lucide="heart" style="width:14px;height:14px;${isFav ? "color:#ff4d6d;fill:#ff4d6d;" : ""}"></i> ${isFav ? "Unfavorite" : "Favorite"} </button> </div>`;
...
list.innerHTML = cart.map(item => ` <div class="cart-item" data-id="${item.id}"> <div class="cart-item-img"> <img src="${item.image}" alt="${item.name}" onerror="this.src='https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=100&q=60'" /> </div> <div class="cart-item-info"> <div class="cart-item-name">${item.name}</div> <div class="cart-item-price">$${(parseFloat(item.price) * item.qty).toFixed(2)}</div> </div> <div class="qty-controls"> <button class="qty-btn minus" data-id="${item.id}" data-delta="-1"> <i data-lucide="minus"></i> </button> <span class="qty-num">${item.qty}</span> <button class="qty-btn plus" data-id="${item.id}" data-delta="1"> <i data-lucide="plus"></i> </button> </div> <button class="remove-btn" data-remove="${item.id}"> <i data-lucide="trash-2"></i> </button> </div>`).join(””);
...
if (discLine) discLine.classList.toggle(“hidden”, discountRate === 0);
}
...
if (receipt) {
receipt.innerHTML = `<div>${itemCount} item${itemCount !== 1 ? "s" : ""} ordered</div> <div>Subtotal: $${subtotal.toFixed(2)}</div> ${discount > 0 ?`<div>Discount: -$${discount.toFixed(2)}</div>`: ""} <div>Delivery: $${DELIVERY_FEE.toFixed(2)}</div> <div class="receipt-total">Total Charged: $${total.toFixed(2)}</div>`;
}
...
pinBuffer += val;
if (pinBuffer.length === 4) setTimeout(() => handlePinKey(“ok”), 200);
}
updatePinDots();
}
...
if (!list) return;
list.innerHTML = products.map(p => ` <div class="cms-item" data-id="${p.id}"> <div class="cms-item-img"> <img src="${p.image}" alt="${p.name}" onerror="this.src='https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=100&q=60'" /> </div> <div class="cms-item-info"> <div class="cms-item-name">${p.name}</div> <div class="cms-item-meta"> <span class="cms-item-cat">${p.category}</span> <span class="cms-item-price">$${parseFloat(p.price).toFixed(2)}</span> <span class="cms-avail-badge ${p.available ? "yes" : "no"}">${p.available ? "On" : "Off"}</span> </div> </div> <div class="cms-item-actions"> <button class="cms-edit-btn" data-id="${p.id}"><i data-lucide="edit"></i></button> <button class="cms-delete-btn" data-id="${p.id}"><i data-lucide="trash-2"></i></button> </div> </div>`).join(””);

list.querySelectorAll(”.cms-edit-btn”).forEach(btn => {
btn.addEventListener(“click”, () => openCMSModal(btn.dataset.id));
});
...
// Cart icon in header
document.getElementById(“cart-icon-btn”)?.addEventListener(“click”, () => showScreen(“cart”));
...
// Modal close buttons
document.getElementById(“modal-close-btn”)?.addEventListener(“click”, () => closeModal(“product-modal”));
document.getElementById(“cms-modal-close”)?.addEventListener(“click”, () => closeModal(“cms-modal”));
document.getElementById(“cms-cancel-btn”)?.addEventListener(“click”, () => closeModal(“cms-modal”));
...
if (e.target === document.getElementById(“cms-modal”)) closeModal(“cms-modal”);
});
document.getElementById(“payment-modal”)?.addEventListener(“click”, e => {
if (e.target === document.getElementById(“payment-modal”)) {
// Don’t close payment modal on backdrop click
}
});
